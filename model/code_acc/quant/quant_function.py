...
def block_minifloat_quantize(x, number, rounding="stochastic", tensor_type="x", dimm=None):
    ...
    max_exponent = block_design(x, number.tile, tensor_type, max_func, dim=dimm) 
    ...
    if number.man == 0:
        ...
        out = i * 2**(max_exponent)
        return out   
    
    # fixed
    elif number.exp == 0:
        bits = number.man + 1
        i = x * 2**(-max_exponent+bits-1)
...
def quantizer(forward_number=None, backward_number=None,
    ...
    tensor_type = "x" #if backward_number is None else "w"
    forward_quant = lambda x, num, rd, tt, d: block_minifloat_quantize(x, num, rd, tt, d)
    backward_quant = lambda x, num, rd, tt, d: block_minifloat_quantize(x, num, rd, tt, d)  
    ...
    class Rounding(torch.autograd.Function):
        ...
        def forward(self, x):
            ...
            out = forward_quant(x.contiguous(), forward_number, forward_rounding, tensor_type, dimm)
            ...
            if self.needs_input_grad[0]:
                if backward_number == None:
                    ...
                    grad_input = backward_quant(grad_output.contiguous(), backward_number, 
                        backward_rounding, tensor_type, dimm)
            else:
                grad_input = None
